...
def organize_cars_by_class(csv_file, source_dir, target_dir, train_ratio=0.8):
    """
    根据CSV文件中的类别标签组织汽车数据
    ...
    for class_id in unique_classes:
        ...
        print(f"\n处理类别 {class_id}: {len(class_images)} 张图片")
...
def create_class_mapping(csv_file, output_file="class_mapping.txt"):
    """
    创建类别映射文件，方便查看类别对应关系
    """
    df = pd.read_csv(csv_file)
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("=" * 30 + "\n")
        for class_id in unique_classes:
            f.write(f"{class_id:3d} -> class_{class_id:03d}\n")
    
    print(f"类别映射文件已保存到: {output_file}")
...
if __name__ == "__main__":
    ...
    # 执行数据整理
    organize_cars_by_class(csv_file, source_dir, target_dir, train_ratio=0.8)
    
    # 创建类别映射文件
    create_class_mapping(csv_file, "data/class_mapping.txt") 